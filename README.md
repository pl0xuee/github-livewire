...
- **Watch anything** — the global firehose by default, or a user (`torvalds`),
  a repo (`rust-lang/rust`), or an org (`org:mozilla`).
...
polls once a minute and streams each batch out over the interval (`304 Not
Modified` responses are free, so quiet targets cost nothing). Drop a
